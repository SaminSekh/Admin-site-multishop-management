...
class ShopProductsViewer {
    ...
    updateDynamicManifest() {
        // If static manifest already linked in HTML (e.g. manifest.json), keep it so browser PWA install criteria are met
        const existingManifest = document.querySelector('link[rel="manifest"]');
        if (existingManifest && !existingManifest.getAttribute('href')?.startsWith('blob:')) {
            return;
        }
